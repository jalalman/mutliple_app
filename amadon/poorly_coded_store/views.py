...
def checkout(request):
    ...
    total_quantity_ordered = Order.objects.aggregate(Sum('quantity_ordered'))['quantity_ordered__sum'] or 0
    total_amount_ordered = float(Order.objects.aggregate(Sum('total_price'))['total_price__sum'] or 0.00)
    formatted_total_charge = f"{total_charge:.2f}"
    formatted_total_amount_ordered = f"{total_amount_ordered:.2f}"
    ...
    request.session['total_quantity_ordered'] = total_quantity_ordered
    request.session['total_amount_ordered'] = formatted_total_amount_ordered
